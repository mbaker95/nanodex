...
1. **NanoDex is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep nanodex (macOS) or systemctl --user status nanodex (Linux)
```
...
| `NANODEX_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/nanodex.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into NanoDex, make the following modifications:
...
All paths below are relative to project root (`NANODEX_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep nanodex  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status nanodex
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/nanodex.log | tail -20

# Script errors
grep -i "error\|failed" logs/nanodex.log | tail -20
```
...
docker run nanodex-agent ls -la /app/src/skills/
```
