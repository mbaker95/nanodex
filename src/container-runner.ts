...
const GROUP_WORKSPACE = `${CONTEXT_ROOT}/group`;
...
function resolveAgentsFile(baseDir: string): string | null {
  const modernPath = path.join(baseDir, 'AGENTS.md');
  if (fs.existsSync(modernPath)) return modernPath;

  const legacyPath = path.join(baseDir, 'CLAUDE.md');
...
function buildVolumeMounts(
  ...
  const globalAgentsFile = resolveAgentsFile(path.join(projectRoot, 'groups', 'global'));
  const groupAgentsFile = resolveAgentsFile(groupDir);
  const groupHasAgentsFile = fs.existsSync(path.join(groupDir, 'AGENTS.md'));
  ...
  mounts.push({
    hostPath: groupDir,
    containerPath: GROUP_WORKSPACE,
    readonly: false,
  });

  if (globalAgentsFile) {
    mounts.push({
      hostPath: globalAgentsFile,
      containerPath: `${CONTEXT_ROOT}/AGENTS.md`,
      readonly: !isMain,
    });
  }

  if (groupAgentsFile && !groupHasAgentsFile) {
    mounts.push({
      hostPath: groupAgentsFile,
      containerPath: `${GROUP_WORKSPACE}/AGENTS.md`,
      readonly: true,
    });
  }
  ...
  if (fs.existsSync(skillsDir)) {
    mounts.push({
      hostPath: skillsDir,
      containerPath: `${CONTEXT_ROOT}/.agents/skills`,
      readonly: true,
    });
  }
...
function buildContainerArgs(
  ...
  if (runtimeEnv.model) {
    args.push('-e', `CODEX_MODEL=${runtimeEnv.model}`);
    args.push('-e', `OPENAI_MODEL=${runtimeEnv.model}`);
  }
    ...
    container.on('close', (code) => {
      ...
      if (isVerbose || isError) {
        ...
        logLines.push(
          ...
          `=== Mounts ===`,
          mounts
            .map((mount) => `${mount.containerPath}${mount.readonly ? ' (ro)' : ''}`)
            .join('\n'),
          ``,
        );
      }
